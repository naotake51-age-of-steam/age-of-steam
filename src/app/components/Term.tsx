import { Title, Stack, List, Text } from "@mantine/core";

// https://kiyaku.jp/hinagata/sns.html から拝借
...
export function Term() {
  const adminName = "蒸気の時代Onlineの管理者";

  return (
    <Stack>
      <Text>
        この利用規約（以下，「本規約」といいます。）は，{adminName}
        （以下，「当社」といいます。）がこのウェブサイト上で提供するサービス（以下，「本サービス」といいます。）の利用条件を定めるものです。登録ユーザーの皆さま（以下，「ユーザー」といいます。）には，本規約に従って，本サービスをご利用いただきます。
      </Text>
      {articles.map(({ section, rules }, idx) => (
        <Article key={idx} number={idx + 1} section={section} rules={rules} />
      ))}
    </Stack>
  );
}
...
function Article({
  ...
  return (
    <Stack gap="sm">
      <Title
        style={{ borderLeft: "5px solid #5db5bd", paddingLeft: "8px" }}
        order={4}
      >
        {`第${String(number)}条（${section}）`}
      </Title>
      <List spacing="xs" className="list-decimal">
        {rules.map((rule, idx) => (
